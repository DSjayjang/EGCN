...
import random
import numpy as np
import util.mol_conv_qm9_hc as mc
...
def collate_emodel_elastic_5(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 1] = mol_graph.Chi0
        self_feats[i, 2] = mol_graph.Kappa1
        self_feats[i, 3] = mol_graph.RingCount
        self_feats[i, 4] = mol_graph.Chi4n
        ####################################################
    graphs, labels = map(list, zip(*samples))
    batched_graph = dgl.batch(graphs)
...
def collate_emodel_elastic_10(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 8] = mol_graph.NumSaturatedCarbocycles
        self_feats[i, 9] = mol_graph.Chi1
        ####################################################
...
def collate_emodel_elastic_20(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 11] = mol_graph.fr_Al_OH
        self_feats[i, 12] = mol_graph.BalabanJ
        self_feats[i, 13] = mol_graph.BCUT2D_CHGHI
        self_feats[i, 14] = mol_graph.BCUT2D_LOGPLOW
        # 16
        self_feats[i, 15] = mol_graph.Chi2n
        self_feats[i, 16] = mol_graph.VSA_EState7
        self_feats[i, 17] = mol_graph.NumRotatableBonds
        self_feats[i, 18] = mol_graph.BCUT2D_LOGPHI
        self_feats[i, 19] = mol_graph.fr_NH1
        ####################################################
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 19] = mol_graph.fr_NH1
        # 21
        self_feats[i, 20] = mol_graph.FpDensityMorgan1
        self_feats[i, 21] = mol_graph.fr_ether
        self_feats[i, 22] = mol_graph.FpDensityMorgan2
        self_feats[i, 23] = mol_graph.Kappa2
        self_feats[i, 24] = mol_graph.SMR_VSA9
        # 26
        self_feats[i, 25] = mol_graph.fr_NH0
        self_feats[i, 26] = mol_graph.SMR_VSA10
        self_feats[i, 27] = mol_graph.fr_Ar_NH
        self_feats[i, 28] = mol_graph.NumSaturatedHeterocycles
        self_feats[i, 29] = mol_graph.BCUT2D_CHGLO
        # 31
        self_feats[i, 30] = mol_graph.NumAromaticRings
        self_feats[i, 31] = mol_graph.VSA_EState8
        self_feats[i, 32] = mol_graph.VSA_EState4
        self_feats[i, 33] = mol_graph.VSA_EState2
        self_feats[i, 34] = mol_graph.SlogP_VSA1
        # 36
        self_feats[i, 35] = mol_graph.SMR_VSA6
        self_feats[i, 36] = mol_graph.fr_C_O
        self_feats[i, 37] = mol_graph.VSA_EState6
        self_feats[i, 38] = mol_graph.BCUT2D_MWLOW
        self_feats[i, 39] = mol_graph.fr_Ar_OH
        # 41
        self_feats[i, 40] = mol_graph.SMR_VSA5
        self_feats[i, 41] = mol_graph.PEOE_VSA4
        self_feats[i, 42] = mol_graph.fr_Al_COO
        self_feats[i, 43] = mol_graph.fr_NH2
        self_feats[i, 44] = mol_graph.VSA_EState5
        # 46
        self_feats[i, 45] = mol_graph.fr_epoxide
        self_feats[i, 46] = mol_graph.fr_term_acetylene
        self_feats[i, 47] = mol_graph.Ipc
        self_feats[i, 48] = mol_graph.fr_oxime
        self_feats[i, 49] = mol_graph.fr_aldehyde
        # 51
        self_feats[i, 50] = mol_graph.EState_VSA9
        self_feats[i, 51] = mol_graph.SlogP_VSA3
        self_feats[i, 52] = mol_graph.fr_quatN
        self_feats[i, 53] = mol_graph.FpDensityMorgan3
        self_feats[i, 54] = mol_graph.PEOE_VSA8
        # 56
        self_feats[i, 55] = mol_graph.fr_Ar_N
        self_feats[i, 56] = mol_graph.EState_VSA1
        self_feats[i, 57] = mol_graph.fr_nitrile
        self_feats[i, 58] = mol_graph.PEOE_VSA3
        self_feats[i, 59] = mol_graph.fr_piperdine
        # 61
        self_feats[i, 60] = mol_graph.fr_aryl_methyl
        self_feats[i, 61] = mol_graph.fr_nitro
        self_feats[i, 62] = mol_graph.PEOE_VSA2
        self_feats[i, 63] = mol_graph.EState_VSA2
        self_feats[i, 64] = mol_graph.fr_unbrch_alkane
        # 66
        self_feats[i, 65] = mol_graph.fr_pyridine
        self_feats[i, 66] = mol_graph.qed
        self_feats[i, 67] = mol_graph.SlogP_VSA10
        self_feats[i, 68] = mol_graph.PEOE_VSA5
        self_feats[i, 69] = mol_graph.PEOE_VSA7
        # 71
        self_feats[i, 70] = mol_graph.fr_bicyclic
        self_feats[i, 71] = mol_graph.EState_VSA7
        ####################################################
