...
import random
import numpy as np
import util.mol_conv_lipo_perm as mc
import util.mol_conv_lipo_all as mc_all
...
def collate_emodel_elastic_3(samples):
    ...
    for i in range(0, len(samples)):
        ...
        ####################################################
        # 1
        self_feats[i, 0] = mol_graph.FpDensityMorgan3
        self_feats[i, 1] = mol_graph.qed
        self_feats[i, 2] = mol_graph.fr_NH2
        ####################################################
...
def collate_emodel_elastic_5(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 1] = mol_graph.qed
        self_feats[i, 2] = mol_graph.fr_NH2
        self_feats[i, 3] = mol_graph.MinEStateIndex
        self_feats[i, 4] = mol_graph.EState_VSA5
        ####################################################
    graphs, labels = map(list, zip(*samples))
    batched_graph = dgl.batch(graphs)
...
def collate_emodel_elastic_7(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 4] = mol_graph.EState_VSA5
        # 6
        self_feats[i, 5] = mol_graph.NumSaturatedCarbocycles
        self_feats[i, 6] = mol_graph.SlogP_VSA4
        ####################################################
...
def collate_emodel_elastic_10(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 5] = mol_graph.NumSaturatedCarbocycles
        self_feats[i, 6] = mol_graph.SlogP_VSA4
        self_feats[i, 7] = mol_graph.fr_piperzine
        self_feats[i, 8] = mol_graph.fr_phenol
        self_feats[i, 9] = mol_graph.fr_HOCCN
        ####################################################
...
#         self_feats[i, 4] = mol_graph.MinAbsEStateIndex
#         # 6
#         self_feats[i, 5] = mol_graph.NumAliphaticHeterocycles
#         self_feats[i, 6] = mol_graph.EState_VSA5
#         self_feats[i, 7] = mol_graph.SlogP_VSA8
#         self_feats[i, 8] = mol_graph.qed
#         self_feats[i, 9] = mol_graph.PEOE_VSA11
#         # 11
#         self_feats[i, 10] = mol_graph.fr_COO
#         self_feats[i, 11] = mol_graph.VSA_EState8
#         self_feats[i, 12] = mol_graph.FpDensityMorgan1
#         self_feats[i, 13] = mol_graph.PEOE_VSA10
#         self_feats[i, 14] = mol_graph.SlogP_VSA4
#         # 16
#         self_feats[i, 15] = mol_graph.RingCount
#         self_feats[i, 16] = mol_graph.SlogP_VSA1
#         self_feats[i, 17] = mol_graph.fr_Nhpyrrole
#         self_feats[i, 18] = mol_graph.fr_azo
#         self_feats[i, 19] = mol_graph.fr_ketone
#         ####################################################
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 9] = mol_graph.fr_HOCCN
        # 11
        self_feats[i, 10] = mol_graph.fr_amidine
        self_feats[i, 11] = mol_graph.fr_furan
        self_feats[i, 12] = mol_graph.fr_guanido
        self_feats[i, 13] = mol_graph.fr_pyridine
        self_feats[i, 14] = mol_graph.MaxEStateIndex
        # 16
        self_feats[i, 15] = mol_graph.PEOE_VSA6
        self_feats[i, 16] = mol_graph.PEOE_VSA12
        self_feats[i, 17] = mol_graph.FpDensityMorgan1
        self_feats[i, 18] = mol_graph.VSA_EState2
        self_feats[i, 19] = mol_graph.SMR_VSA1
        ####################################################
...
def collate_emodel_all(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 3] = mol_graph.qed
        self_feats[i, 4] = mol_graph.FpDensityMorgan1
        # 6
        self_feats[i, 5] = mol_graph.FpDensityMorgan3
        self_feats[i, 6] = mol_graph.Chi4v
        self_feats[i, 7] = mol_graph.Ipc
        self_feats[i, 8] = mol_graph.PEOE_VSA12
        self_feats[i, 9] = mol_graph.PEOE_VSA13
        # 11
        self_feats[i, 10] = mol_graph.PEOE_VSA6
        self_feats[i, 11] = mol_graph.SMR_VSA1
        self_feats[i, 12] = mol_graph.SMR_VSA10
        self_feats[i, 13] = mol_graph.SlogP_VSA10
        self_feats[i, 14] = mol_graph.SlogP_VSA4
        # 16
        self_feats[i, 15] = mol_graph.EState_VSA5
        self_feats[i, 16] = mol_graph.VSA_EState2
        self_feats[i, 17] = mol_graph.VSA_EState5
        self_feats[i, 18] = mol_graph.NumAromaticHeterocycles
        self_feats[i, 19] = mol_graph.NumHAcceptors
        # 21
        self_feats[i, 20] = mol_graph.NumSaturatedCarbocycles
        self_feats[i, 21] = mol_graph.MolLogP
        self_feats[i, 22] = mol_graph.fr_COO
        self_feats[i, 23] = mol_graph.fr_HOCCN
        self_feats[i, 24] = mol_graph.fr_NH1
        # 26
        self_feats[i, 25] = mol_graph.fr_NH2
        self_feats[i, 26] = mol_graph.fr_amidine
        self_feats[i, 27] = mol_graph.fr_bicyclic
        self_feats[i, 28] = mol_graph.fr_furan
        self_feats[i, 29] = mol_graph.fr_guanido
        # 31
        self_feats[i, 30] = mol_graph.fr_lactone
        self_feats[i, 31] = mol_graph.fr_nitro_arom_nonortho
        self_feats[i, 32] = mol_graph.fr_phenol
        self_feats[i, 33] = mol_graph.fr_piperzine
        self_feats[i, 34] = mol_graph.fr_pyridine
        # 36
        self_feats[i, 35] = mol_graph.fr_sulfide
        self_feats[i, 36] = mol_graph.fr_unbrch_alkane
        ####################################################
