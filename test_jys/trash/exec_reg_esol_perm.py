...
import random
import numpy as np
import util.mol_conv_esol_perm as mc
import util.mol_conv_esol_all as mc_all
...
def collate_emodel_elastic_3(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 2] = mol_graph.BCUT2D_MWLOW
        ####################################################
...
def collate_emodel_elastic_10(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 8] = mol_graph.fr_bicyclic
        self_feats[i, 9] = mol_graph.SlogP_VSA4
        ####################################################
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 9] = mol_graph.SlogP_VSA4
        # 11
        self_feats[i, 10] = mol_graph.EState_VSA8
        self_feats[i, 11] = mol_graph.SMR_VSA9
        self_feats[i, 12] = mol_graph.FractionCSP3
        self_feats[i, 13] = mol_graph.EState_VSA5
        self_feats[i, 14] = mol_graph.fr_para_hydroxylation
        # 16
        self_feats[i, 15] = mol_graph.SlogP_VSA7
        self_feats[i, 16] = mol_graph.FpDensityMorgan2
        ####################################################
...
def collate_emodel_all(samples):
    ...
    for i in range(0, len(samples)):
        ...
        # 16
        self_feats[i, 15] = mol_graph.SlogP_VSA4
        self_feats[i, 16] = mol_graph.SlogP_VSA7
        self_feats[i, 17] = mol_graph.EState_VSA10
        self_feats[i, 18] = mol_graph.EState_VSA11
        self_feats[i, 19] = mol_graph.EState_VSA2
        # 21
        self_feats[i, 20] = mol_graph.EState_VSA5
        self_feats[i, 21] = mol_graph.EState_VSA8
        self_feats[i, 22] = mol_graph.FractionCSP3
        self_feats[i, 23] = mol_graph.NumAromaticHeterocycles
        self_feats[i, 24] = mol_graph.NumSaturatedRings
        # 26
        self_feats[i, 25] = mol_graph.MolLogP
        self_feats[i, 26] = mol_graph.fr_Al_OH_noTert
        self_feats[i, 27] = mol_graph.fr_Ar_NH
        self_feats[i, 28] = mol_graph.fr_C_O
        self_feats[i, 29] = mol_graph.fr_NH0
        # 31
        self_feats[i, 30] = mol_graph.fr_alkyl_halide
        self_feats[i, 31] = mol_graph.fr_amide
        self_feats[i, 32] = mol_graph.fr_aryl_methyl
        self_feats[i, 33] = mol_graph.fr_barbitur
        self_feats[i, 34] = mol_graph.fr_bicyclic
        # 36
        self_feats[i, 35] = mol_graph.fr_ester
        self_feats[i, 36] = mol_graph.fr_furan
        self_feats[i, 37] = mol_graph.fr_hdrzine
        self_feats[i, 38] = mol_graph.fr_imidazole
        self_feats[i, 39] = mol_graph.fr_imide
        # 41
        self_feats[i, 40] = mol_graph.fr_nitro_arom_nonortho
        self_feats[i, 41] = mol_graph.fr_para_hydroxylation
        self_feats[i, 42] = mol_graph.fr_phos_acid
        ####################################################
