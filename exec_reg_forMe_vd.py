...
def collate_emodel_elastic_3(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 1] = mol_graph.VSA_EState7
        self_feats[i, 2] = mol_graph.Chi4v
        ####################################################
...
def collate_emodel_elastic_7(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 4] = mol_graph.HeavyAtomMolWt
        # 6
        self_feats[i, 5] = mol_graph.PEOE_VSA4
...
def collate_emodel_elastic_10(samples):
    self_feats = np.empty((len(samples), 9), dtype=np.float32)
    ...
    for i in range(0, len(samples)):
        ...
        # self_feats[i, 8] = mol_graph.BCUT2D_MWHI
        ####################################################
...
#         self_feats[i, 7] = mol_graph.MolMR
#         self_feats[i, 8] = mol_graph.fr_allylic_oxid
#         self_feats[i, 9] = mol_graph.BCUT2D_MWHI
#         # 11
#         self_feats[i, 10] = mol_graph.EState_VSA6
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        # self_feats[i, 9] = mol_graph.BCUT2D_MWHI
        # 11
        self_feats[i, 9] = mol_graph.EState_VSA6
        self_feats[i, 10] = mol_graph.PEOE_VSA8
        self_feats[i, 11] = mol_graph.NumHDonors
...
model_EGCN_10 = EGCN_10_copy.Net(mc.dim_atomic_feat, 1, 9).to(device)
model_EGCN_20 = Outer_EGCN_elastic_copy.Net(mc.dim_atomic_feat, 1, 13).to(device)
...
model_Outer_EGCN_10 = Outer_EGCN_10_copy.Net(mc.dim_atomic_feat, 1, 9).to(device)
model_Outer_EGCN_20 = Outer_EGCN_elastic_copy.Net(mc.dim_atomic_feat, 1, 13).to(device)
...
# # feature 20개
# print('--------- EGCN_20 ---------')
# test_losses['EGCN_20'] = trainer.cross_validation(dataset, model_EGCN_20, criterion, k, batch_size, max_epochs, trainer.train_emodel, trainer.test_emodel, collate_emodel_elastic)
