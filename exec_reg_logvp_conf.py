...
import random
import numpy as np
import util.mol_conv_logvp_conf as mc
...
def collate_emodel_elastic_5(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 4] = mol_graph.EState_VSA1
        ####################################################
    graphs, labels = map(list, zip(*samples))
    batched_graph = dgl.batch(graphs)
...
def collate_emodel_elastic_10(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 8] = mol_graph.NumAliphaticHeterocycles
        self_feats[i, 9] = mol_graph.EState_VSA5
        ####################################################
...
def collate_emodel_elastic_20(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 11] = mol_graph.fr_amidine
        self_feats[i, 12] = mol_graph.VSA_EState9
        self_feats[i, 13] = mol_graph.SlogP_VSA8
        self_feats[i, 14] = mol_graph.EState_VSA9
        # 16
        self_feats[i, 15] = mol_graph.qed
        self_feats[i, 16] = mol_graph.PEOE_VSA11
        self_feats[i, 17] = mol_graph.fr_COO
        self_feats[i, 18] = mol_graph.FpDensityMorgan1
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 19] = mol_graph.VSA_EState8
        # 21
        self_feats[i, 20] = mol_graph.SMR_VSA4
        self_feats[i, 21] = mol_graph.fr_ketone_Topliss
        self_feats[i, 22] = mol_graph.PEOE_VSA10
        self_feats[i, 23] = mol_graph.SlogP_VSA4
        self_feats[i, 24] = mol_graph.RingCount
        # 26
        self_feats[i, 25] = mol_graph.fr_Nhpyrrole
        self_feats[i, 26] = mol_graph.fr_Al_OH_noTert
        self_feats[i, 27] = mol_graph.NumSaturatedHeterocycles
        self_feats[i, 28] = mol_graph.fr_azo
        self_feats[i, 29] = mol_graph.fr_lactone
        # 31
        self_feats[i, 30] = mol_graph.fr_NH1
        self_feats[i, 31] = mol_graph.fr_nitrile
        self_feats[i, 32] = mol_graph.fr_piperdine
