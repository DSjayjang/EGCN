...
import random
import numpy as np
import util.mol_conv_freesolv_conf as mc
...
def collate_emodel_elastic_20(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 12] = mol_graph.SlogP_VSA4
        self_feats[i, 13] = mol_graph.fr_Al_OH
        self_feats[i, 14] = mol_graph.NumHDonors
        # 16
        self_feats[i, 15] = mol_graph.VSA_EState8
        self_feats[i, 16] = mol_graph.MinPartialCharge
        self_feats[i, 17] = mol_graph.RingCount
        self_feats[i, 18] = mol_graph.fr_Al_COO
        self_feats[i, 19] = mol_graph.fr_aryl_methyl
        ####################################################
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 22] = mol_graph.fr_bicyclic
        self_feats[i, 23] = mol_graph.Ipc
        ####################################################
...
# permutation importance - confidence interval (p-value)
