...
import random
import numpy as np
import util.mol_conv_esol_conf as mc
...
def collate_emodel_elastic_7(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 6] = mol_graph.MinAbsPartialCharge
        ####################################################
...
def collate_emodel_elastic_10(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 8] = mol_graph.BalabanJ
...
def collate_emodel_elastic_20(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 9] = mol_graph.SlogP_VSA1
        # 11
        self_feats[i, 10] = mol_graph.fr_Ar_NH
        self_feats[i, 11] = mol_graph.fr_imide
        self_feats[i, 12] = mol_graph.NumAromaticHeterocycles
        self_feats[i, 13] = mol_graph.fr_NH0
        self_feats[i, 14] = mol_graph.SlogP_VSA10
        # 16
        self_feats[i, 15] = mol_graph.fr_amide
        self_feats[i, 16] = mol_graph.fr_ester
        self_feats[i, 17] = mol_graph.PEOE_VSA13
        self_feats[i, 18] = mol_graph.fr_bicyclic
        self_feats[i, 19] = mol_graph.EState_VSA10
        ####################################################
...
def collate_emodel_elastic(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 19] = mol_graph.EState_VSA10
        # 21
        self_feats[i, 20] = mol_graph.NumSaturatedRings
        self_feats[i, 21] = mol_graph.PEOE_VSA9
        self_feats[i, 22] = mol_graph.fr_barbitur
        self_feats[i, 23] = mol_graph.fr_C_O
        self_feats[i, 24] = mol_graph.fr_alkyl_halide
        # 26
        self_feats[i, 25] = mol_graph.SlogP_VSA4
        self_feats[i, 26] = mol_graph.EState_VSA8
        self_feats[i, 27] = mol_graph.SMR_VSA9
        self_feats[i, 28] = mol_graph.fr_imidazole
        self_feats[i, 29] = mol_graph.fr_para_hydroxylation
        # 31
        self_feats[i, 30] = mol_graph.fr_nitro_arom_nonortho
        ####################################################
